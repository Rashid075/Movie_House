...
const Movie = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        setNoResults(
          response.data.results.length === 0 && search.trim() !== ""
        );
      } catch (error) {
    ...
    if (query) {
      try {
        const response = await axios.get(
          ...
          {
            params: { api_key: import.meta.env.VITE_API_KEY, query },
          }
        );
  ...
  return (
    <div className="bg-gray-900 min-h-screen text-gray-100">
      <h1 className="text-center text-6xl p-10 text-blue-400 font-bold drop-shadow-lg">
        MovieHouse
      </h1>
      <div className="flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6 p-4">
          {(showFavorites ? favorites : movies).length > 0 ? (
            ...
            <p className="text-center text-gray-400 col-span-full text-5xl p-5">
